...
function getShopDetails() {
    ...
    $.ajax({
        type: 'get',
        url: $('meta[name="route-get-shop-details"]').attr("content"),
        // data: formData,
        processData: false,
        contentType: false,
        dataType: 'json',
        ...
        success: function(response) {
            if (response.success === true) { // Vérifie la propriété 'success' du JSON du contrôleur Laravel
                // alert(JSON.stringify(response.products));
...
function createBlocDetailsStructure(details) {
    ...
    details.forEach((element, index)=>{
        ...
        <div>
            ...
            <p class="text-[11px] text-gray-700">${element.description}</p>
        </div>
     `
...
function createBlocDetailsProducts(products) {
    ...
    products.forEach((element, index) => {
        ...
        divSubContainer.innerHTML = `
            ...
            <p class="text-[13px] ">
                ${element.quantite > 0
            ? `<span class="bg-green-300 py-[1px] px-2 font-bold">En Stock : ${element.quantite}</span>`
            : `<span class="bg-red-300 py-[1px] px-2 font-bold">Épuisé</span>`}
            </p>
            `;
